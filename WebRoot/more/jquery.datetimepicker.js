...
(function($) {
	...
	$.fn.xdsoftScroller = function(_percent) {
		return this.each(function() {
			...
			if (!$(this).hasClass('xdsoft_scroller_box')) {
				...
				scroller.on('mousedown.xdsoft_scroller',
					function(event) {
						...
						$([document.body, window]).on('mouseup.xdsoft_scroller',
				...
				timeboxparent.on('scroll_element.xdsoft_scroller',
	...
	$.fn.datetimepicker = function(opt) {
		var KEY0 = 48,
			...
			YKEY = 89,
			ctrlDown = false,
			options = ($.isPlainObject(opt) || !opt) ? $.extend(true, {},
			...
			createDateTimePicker = function(input) {
				...
				mounth_picker.find('.xdsoft_month,.xdsoft_year').on('mousedown.xdsoft',
					function(event) {
				...
				mounth_picker.find('.xdsoft_select').xdsoftScroller().on('mousedown.xdsoft',
					function(event) {
						event.stopPropagation();
						event.preventDefault();
				...
				datetimepicker.setOptions = function(_options) {
								...
								input.on('keydown.xdsoft',
									function(event) {
										var val = this.value,
											key = event.which;
										switch (true) {
											case((key >= KEY0 && key <= KEY9) || (key >= _KEY0 && key <= _KEY9)) || (key == BACKSPACE || key == DEL) : var pos = getCaretPos(this),
					...
					if (options.validateOnBlur) {
						input.off('blur.xdsoft').on('blur.xdsoft',
							function() {
				...
				mounth_picker.find('.xdsoft_prev,.xdsoft_next').on('mousedown.xdsoft',
					function() {
						var $this = $(this),
							timer = 0,
							stop = false;
						...
						$([document.body, window]).on('mouseup.xdsoft',
				...
				timepicker.find('.xdsoft_prev,.xdsoft_next').on('mousedown.xdsoft',
					function() {
						var $this = $(this),
							timer = 0,
							stop = false,
							...
							period = (period > 10) ? 10 : period - 10; ! stop && (timer = setTimeout(arguments_callee4, v ? v: period));
						})(500);
						$([document.body, window]).on('mouseup.xdsoft',
				...
				datetimepicker.on('xchange.xdsoft',
					function(event) {
						var table = '',
				...
				calendar.on('click.xdsoft', 'td',
					function() {
						...
						currentTime.setFullYear($this.data('year'),$this.data('month'),$this.data('date'));
				...
				datetimepicker.on('changedatetime.xdsoft',
					function() {
					...
					function() {
						...
						if (onShow !== false) {
							...
							if (options.closeOnWithoutClick) {
								$([document.body, window]).on('mousedown.xdsoft',
				...
				input.data('xdsoft_datetimepicker', datetimepicker).on('open.xdsoft focusin.xdsoft mousedown.xdsoft',
					function(event) {
		...
		$(document).off('keydown.xdsoftctrl keyup.xdsoftctrl').on('keydown.xdsoftctrl',
			function(e) {
